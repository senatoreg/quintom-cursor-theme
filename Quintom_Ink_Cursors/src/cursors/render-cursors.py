	...
	def renderFromSVG(self, svgFName, slicename, skipped, roundrobin, hotsvgFName):
		...
		def do_res (size, output, svgFName, skipped, roundrobin):
			if os.path.exists (output):
				skipped[output] = True
				return
			command = 'inkscape -w {size} -h {size} --export-id="{export_id}" --export-filename="{export_png}" {svg}\n'.format (size=size, export_id=self.name, export_png=output, svg=svgFName).encode('utf-8')
			info("Processing: {}".format (self.name))
